...
| 工作台首页同时展示“固定评测结果”与“进程内实时运行指标” | 前者适合答辩和回归对比，后者适合部署验收和线上排障，两者解决的问题不同，不能互相替代 |
| 评测脚本正式覆盖 Agent 成功、Run 回放、工具调用和授权判定，而不再只看检索命中 | P0 收口阶段需要证明多智能体主线确实可执行、可审计，而不是只证明“搜到了知识” |
| 软件杯 PRD 与技术规划必须显式纳入 `LoongArch + 银河麒麟`、`4 核 CPU + 8GB 内存 + 256GB 硬盘` 约束 | 这些不是可选背景信息，而是会直接改变模型部署、检索底座和交付优先级的硬条件 |
| 比赛阶段技术路线继续坚持“云端模型优先 + PostgreSQL 轻量 RAG + 硬状态机证据门禁” | 在受限硬件和交付期限下，这一组合比本地大模型、Milvus、Neo4j、RLHF 更能形成稳定完整的工程闭环 |
