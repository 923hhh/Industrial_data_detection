...
- 下一轮增强目标：
  - 增加 OCR，使扫描件和图片型 PDF 可导入
  - 增加导入预览、冲突检测、导入记录和来源回溯
  - 区分“文档导入上传”和“故障图上传”两类入口
...
- 下一轮增强目标：
  - 增加工单编号、设备编号、设备型号、优先级、报修来源等工单/设备信息
  - 将“检修步骤输出”从辅助结果升级为正式结构化层，包含工具、材料、风险和确认动作
  - 增加检修流程推荐，让系统能输出完整作业路径而不仅是单步建议
